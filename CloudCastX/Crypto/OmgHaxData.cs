using System;
using System.IO;
using System.Linq;
using System.Threading.Tasks;
using Windows.Storage;

namespace CloudCast.Crypto
{
    // Static data tables and keys for the FairPlay white-box AES decryption.
    //
    // Ported from SteeBono/airplayreceiver (MIT License):
    //   https://github.com/SteeBono/airplayreceiver
    internal static class OmgHaxData
    {
        public static readonly byte[] StaticSource1 = {
            0xFA, 0x9C, 0xAD, 0x4D, 0x4B, 0x68, 0x26, 0x8C,
            0x7F, 0xF3, 0x88, 0x99, 0xDE, 0x92, 0x2E, 0x95, 0x1E
        };

        public static readonly byte[] StaticSource2 = {
            0xEC, 0x4E, 0x27, 0x5E, 0xFD, 0xF2, 0xE8, 0x30,
            0x97, 0xAE, 0x70, 0xFB, 0xE0, 0x00, 0x3F, 0x1C,
            0x39, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10,
            0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
        };

        public static readonly byte[] InitialSessionKey = {
            0xDC, 0xDC, 0xF3, 0xB9, 0x0B, 0x74, 0xDC, 0xFB,
            0x86, 0x7F, 0xF7, 0x60, 0x16, 0x72, 0x90, 0x51
        };

        public static readonly byte[] DefaultSap = {
            
        };

        public static readonly byte[] IndexMAngle = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C };

        public static readonly byte[][] MessageKey = {
            new byte[] {  },
            new byte[] {  },
            new byte[] {  },
            new byte[] {  }
        };

        public static readonly byte[][] MessageIv = {
            new byte[] { 0x57, 0x52, 0xF1, 0xB7, 0x54, 0x9D, 0x8F, 0x87, 0x0C, 0x10, 0x48, 0x5A, 0x60, 0x88, 0xCA, 0xDB },
            new byte[] { 0xDF, 0x7B, 0x15, 0x63, 0xF0, 0x05, 0x58, 0x77, 0x52, 0xA9, 0x04, 0x02, 0xB9, 0xA3, 0x92, 0x95 },
            new byte[] { 0x68, 0xB5, 0x46, 0x11, 0xFB, 0x04, 0xDE, 0x67, 0x6C, 0x96, 0x8E, 0xFB, 0x8C, 0x9D, 0xB0, 0xC9 },
            new byte[] { 0x27, 0x07, 0x8B, 0x21, 0x23, 0x36, 0x1E, 0x7A, 0xDC, 0x9D, 0x0B, 0x11, 0x53, 0x54, 0x69, 0x0D }
        };

        public static readonly byte[] TKey = { 0xd0, 0x04, 0xa9, 0x61, 0x6b, 0xa4, 0x00, 0x87, 0x68, 0x8b, 0x5f, 0x15, 0x15, 0x35, 0xd9, 0xa9 };
        public static readonly byte[] XKey = { 0x8e, 0xba, 0x07, 0xcc, 0xb6, 0x5a, 0xf6, 0x20, 0x33, 0xcf, 0xf8, 0x42, 0xe5, 0xd5, 0x5a, 0x7d };
        public static readonly byte[] ZKey = { 0x1a, 0x64, 0xf9, 0x60, 0x6c, 0xe3, 0x01, 0xa9, 0x54, 0x48, 0x1b, 0xd4, 0xab, 0x81, 0xfc, 0xc6 };

        // Lookup tables loaded from resource files at startup via LoadTablesAsync
        public static byte[] TableS1 = null!;
        public static byte[] TableS2 = null!;
        public static byte[] TableS3 = null!;
        public static byte[] TableS4 = null!;
        public static int[] TableS5 = null!;
        public static int[] TableS6 = null!;
        public static int[] TableS7 = null!;
        public static int[] TableS8 = null!;
        public static int[] TableS9 = null!;
        public static byte[] TableS10 = null!;

        public static bool IsLoaded { get; private set; }

        public static async Task LoadTablesAsync()
        {
            if (IsLoaded) return;

            var folder = await Windows.ApplicationModel.Package.Current.InstalledLocation
                .GetFolderAsync("Crypto");
            var resFolder = await folder.GetFolderAsync("Resources");

            TableS1  = await ReadTableBytesAsync(resFolder, "table_s1.bin");
            TableS2  = await ReadTableBytesAsync(resFolder, "table_s2.bin");
            TableS3  = await ReadTableBytesAsync(resFolder, "table_s3.bin");
            TableS4  = await ReadTableBytesAsync(resFolder, "table_s4.bin");
            TableS5  = await ReadTableIntsAsync(resFolder, "table_s5.bin");
            TableS6  = await ReadTableIntsAsync(resFolder, "table_s6.bin");
            TableS7  = await ReadTableIntsAsync(resFolder, "table_s7.bin");
            TableS8  = await ReadTableIntsAsync(resFolder, "table_s8.bin");
            TableS9  = await ReadTableIntsAsync(resFolder, "table_s9.bin");
            TableS10 = await ReadTableBytesAsync(resFolder, "table_s10.bin");

            IsLoaded = true;
            System.Diagnostics.Debug.WriteLine("[OmgHax] Lookup tables loaded");
        }

        private static async Task<byte[]> ReadTableBytesAsync(StorageFolder folder, string name)
        {
            var file = await folder.GetFileAsync(name);
            var text = await FileIO.ReadTextAsync(file);
            return text.Split(',')
                .Where(h => !string.IsNullOrWhiteSpace(h))
                .Select(h => Convert.ToByte(h.Replace("0x", "").Trim(), 16))
                .ToArray();
        }

        private static async Task<int[]> ReadTableIntsAsync(StorageFolder folder, string name)
        {
            var file = await folder.GetFileAsync(name);
            var text = await FileIO.ReadTextAsync(file);
            return text.Split(',')
                .Where(h => !string.IsNullOrWhiteSpace(h))
                .Select(h => Convert.ToInt32(h.Replace("0x", "").Trim(), 16))
                .ToArray();
        }
    }
}
