using System;

namespace CloudCast.Services
{
    // FairPlay 3 session-key exchange for the /fp-setup endpoint.
    //
    // Ported from SteeBono/airplayreceiver (MIT License):
    //   https://github.com/SteeBono/airplayreceiver
    //
    // The exchange has two phases:
    //   Phase 1: client sends 16 bytes (contains FP version + mode), server replies 142 bytes.
    //   Phase 2: client sends 164 bytes (key message), server echoes last 20 bytes with FPLY header.
    //
    // The 164-byte key message from phase 2 is stored and later used by OmgHax
    // to decrypt the AES session key for video stream decryption.
    internal class FairPlaySetup
    {
        private static readonly byte[] FpHeader = {
            0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00,
            0x00, 0x00, 0x00, 0x14
        };

        private static readonly byte[][] ReplyMessages = {
            new byte[] {
                
            },
            new byte[] {
                
            },
            new byte[] {
                
            },
            new byte[] {
                
            }
        };

        public byte[]? KeyMsg { get; private set; }

        public byte[]? Handle(byte[] request)
        {
            if (request.Length == 16)
                return HandlePhase1(request);
            if (request.Length == 164)
                return HandlePhase2(request);

            System.Diagnostics.Debug.WriteLine(
                $"[FairPlay] Unexpected /fp-setup body length: {request.Length}");
            return null;
        }

        private byte[]? HandlePhase1(byte[] body)
        {
            if (body[4] != 0x03)
            {
                System.Diagnostics.Debug.WriteLine(
                    $"[FairPlay] Unsupported FairPlay version: {body[4]}");
                return null;
            }

            int mode = body[14];
            if (mode < 0 || mode > 3)
            {
                System.Diagnostics.Debug.WriteLine(
                    $"[FairPlay] Unsupported mode: {mode}");
                return null;
            }

            System.Diagnostics.Debug.WriteLine($"[FairPlay] Phase 1 — mode {mode}");
            return ReplyMessages[mode];
        }

        private byte[] HandlePhase2(byte[] body)
        {
            if (body[4] != 0x03)
            {
                System.Diagnostics.Debug.WriteLine(
                    $"[FairPlay] Phase 2 unsupported version: {body[4]}");
                return null;
            }

            KeyMsg = new byte[164];
            Buffer.BlockCopy(body, 0, KeyMsg, 0, 164);

            var output = new byte[32];
            Buffer.BlockCopy(FpHeader, 0, output, 0, 12);
            Buffer.BlockCopy(body, 144, output, 12, 20);

            System.Diagnostics.Debug.WriteLine("[FairPlay] Phase 2 — key message stored");
            return output;
        }
    }
}
